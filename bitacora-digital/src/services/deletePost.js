/*
 Este fichero conecta con el recurso DELETE de la API.
 Realiza el BORRADO de datos eliminando un REGISTRO en la API mediante el método DELETE. 
*/
...
//configuración de la petición DELETE que necesita fetch para enviar los datos.
//creamos el objeto que enviaremos con la informacion del form y lo metemos en la petición http
/*const requestData = { 
    method: 'DELETE', // crea un nuevo registro
...
//=================== API FETCH CON .THEN ()====================
//creo una funcion que devolverá la ELIMINACIÓN de datos en la API el recurso de DELETE. (y todas sus propiedades )
//paso como parametros la id del post(para la url).
function deletePost (id) {
    ...
    return  (
        fetch( `${ApiPost}/${id}`, {
            method: 'DELETE', // eliminamos el registro completo sin necesidad de enviar el objeto completo solo con la id. 
            //body: JSON.stringify(),
            headers: {   //le dice al servidor que le envia el JSON. 
                'Content-Type': 'application/json; charset=UTF-8',
            },
